...
async function bundle(entryPath, projectDir) {
  ...
  await esbuild.build({
    ...
    // Platform "neutral" ignores package.json#main unless mainFields is set
    // explicitly. Many npm packages still ship CJS-only with just `main`
    // (node-forge, elliptic, bip39, …), so we include it here. `module` and
    // `browser` come first so we get ESM / browser-shaped builds when
    // available.
...
/**
 * esbuild plugin: resolve Node-only built-ins (`fs`, `path`, `net`, `tls`,
 * `child_process`, `ws`, …) to a synthetic stub module that succeeds at
 * module-init but throws a clear error if the program actually reaches a
 * Node primitive.
 *
 * Why this exists: a lot of npm libraries ship Node-targeted bundles that
 * pull in `fs` / `path` at the top of the file but only call into them
 * inside specific class methods. If a capsule consumes such a library and
 * never reaches those methods, the imports are dead weight that esbuild
 * still has to resolve. This plugin lets resolution succeed without
 * silently masking real Node-only usage at runtime — if the dead-code
 * assumption is wrong, the stub throws with a self-explanatory message
 * naming the call site.
 *
 * Note on `crypto` / `buffer` / `events` / `stream` / `util`: these are
 * stubbed too. StarlingMonkey already provides browser-shape equivalents
 * on globalThis (`crypto.subtle`, `crypto.getRandomValues`, `Buffer` via
 * SDK polyfill, `EventTarget`, …). User code that reaches for the Node
 * module by name is using the wrong shape; the stub lets the import
 * resolve so dead-code paths inside third-party libs still bundle, and
 * throws with a clear message if a real call site reaches it.
 */
function nodeBuiltinStubPlugin() {
  const STUBBED = [
    ...
    "domain",
    "inspector",
    // Third-party Node-only packages we treat the same way.
    "ws",
  ];
  const stubbed = new Set(STUBBED.flatMap((name) => [name, `node:${name}`]));
  const STUB_NS = "astrid-build-node-stub";
  // Loaded as CommonJS — esbuild's CJS↔ESM interop lets `import { Buffer }
  // from "buffer"` and `import * as fs from "fs"` both resolve through
  // `module.exports = trap`. The trap is a Proxy that yields throwing
  // functions for every property access, so the import statement succeeds,
  // tree-shaking proceeds, and only an actual call site explodes.
  const stubSource = (importer) => `// auto-generated by astrid-build's node-builtin-stub plugin
const importer = ${JSON.stringify(importer)};
function explode(prop) {
  throw new Error(
    "astrid capsule: Node-only API '" + prop + "' is not available in a " +
    "wasip2 component. Reached via import in " + importer + ". Use the " +
    "@astrid-os/sdk equivalent (fs, net, http, process, ...) or vendor a " +
    "user-space replacement that doesn't depend on Node built-ins."
  );
}
const leaf = (name) => new Proxy(function () { explode(name); }, {
  get(_, p) {
    if (p === "__esModule") return true;
    if (p === "default") return leaf(name);
    if (typeof p === "string") return leaf(name + "." + p);
    return undefined;
  },
});
const trap = new Proxy(function () { explode("(call)"); }, {
  get(_, key) {
    if (key === "__esModule") return true;
    if (key === "default") return trap;
...
`;
  return {
    name: "astrid-build-node-stub",
    setup(build) {
      build.onResolve({ filter: /.*/ }, (args) => {
        if (stubbed.has(args.path)) {
          return { path: args.path, namespace: STUB_NS, pluginData: { importer: args.importer } };
        }
        return null;
      });
      build.onLoad({ filter: /.*/, namespace: STUB_NS }, (args) => {
        return {
          contents: stubSource(args.pluginData?.importer ?? "<unknown>"),
          loader: "js",
          // Use the project's resolveDir so esbuild's CJS-to-ESM helper has
          // a stable working directory.
          resolveDir: dirname(args.pluginData?.importer ?? build.initialOptions.absWorkingDir ?? "."),
        };
      });
    },
  };
